...
mod tests {
    use super::{
        Arc, BACKEND_ID, BufferSize, CHANNELS, DEFAULT_BUFFER_FRAMES, DeviceTrait, Duration,
        ErrorKind, HostTrait, Instant, MAX_BUFFER_FRAMES, MIN_BUFFER_FRAMES, MockDeviceKind,
        MockHost, Mutex, Ordering, SAMPLE_FORMAT, SAMPLE_RATE, StreamConfig, StreamTrait,
        SupportedBufferSize, host, is_mock_backend, negotiate_frames, thread,
    };
    ...
    fn negotiates_the_requested_block_size_within_the_supported_range() {
        assert_eq!(negotiate_frames(&fixed_config(128), SAMPLE_FORMAT).unwrap(), 128);
        assert_eq!(
