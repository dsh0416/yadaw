...
use heron_dsp_runtime::protocol::{
    AudioEngineConfig, BinaryPayload, ControlCommand, ControlResponse, ControlResult,
    GraphTransactionRequest, GraphUpdate, HostEvent, INLINE_BLOB_LIMIT, LiveMixerGraph,
    LiveTempoEvent, LiveTimeSignatureEvent, MidiSyncPreferences, MixerParameterPreview,
    ParameterGesture, PluginAudioMode, PluginEditorContext, PluginEditorPreference,
    PrepareGraphRequest, PriorityCommand, PriorityRequest, PriorityResponse, PriorityResult,
...
use heron_ipc_transport::{
    ArenaReceiver, LeaseRegistry, MappingCommand, MappingEvent, ParameterProducer, RegionOffer,
    TelemetryReader, TelemetryWriter, WirePacket, create_parameter_ring, create_telemetry_page,
    decode_body, encode_body, encode_priority, encode_response,
};
use ipc_channel::ipc;
use napi::{Error, Status};

use super::{
    OUTBOUND_CAPACITY, ROUTER_POLL,
    egress::spawn_egress,
    event_router::spawn_event_router,
    lease_release::{
        expire_pending, reject_all, resolve_pending, router_timeout, send_release_leases,
    },
    priority_router::spawn_priority_router,
    request_routing::{queue_pending_request, register_pending},
    response_router::spawn_response_router,
    routing_helpers::{parse_gesture, record_packet, request_deadline},
    state::{
        Pending, PendingResponder, TransportTraffic, decode_native_window_handle, failure,
        negotiate_shared_pages, resolve_runtime_config,
    },
};
...
fn wait_for_queued_event(events: &Mutex<VecDeque<Vec<u8>>>) -> Vec<u8> {
    ...
    loop {
        ...
        assert!(
            ...
            "event router did not enqueue an event"
        );
        thread::yield_now();
    }
}
...
fn shared_page_negotiation_activates_only_after_two_way_verification() {
    ...
    let peer_telemetry =
        heron_ipc_transport::TelemetryWriter::open_and_acknowledge(telemetry.descriptor())
            .expect("peer telemetry writer");
    let peer_parameters =
        heron_ipc_transport::ParameterConsumer::open_and_acknowledge(parameters.descriptor())
...
fn pending_registration_preserves_the_original_request_on_duplicate_id() {
    ...
    let (duplicate, duplicate_outcome) = test_pending(Instant::now() + Duration::from_secs(1));
...
fn pending_registration_enforces_the_outbound_capacity() {
    ...
    let error =
        register_pending(&pending, u64::MAX, overflow).expect_err("reject request beyond capacity");
...
fn disconnected_outbound_rejects_and_removes_the_registered_request() {
    ...
    queue_pending_request(&pending, &Mutex::new(Some(sender)), 11, packet(vec![1]))
...
fn expiry_rejects_only_elapsed_requests_and_counts_each_timeout() {
    let pending = Mutex::new(HashMap::new());
    let (expired, expired_outcome) = test_pending(Instant::now() - Duration::from_millis(1));
...
fn priority_router_resolves_valid_packet_and_rejects_invalid_packet() {
    ...
    sender
        .send(packet(vec![0xc1]))
        .expect("send invalid priority packet");
    assert!(matches!(
...
fn event_router_bounds_fifo_and_keeps_the_newest_events() {
    ...
    for revision in 0..=OUTBOUND_CAPACITY as u64 {
        sender
            .send(packet(
                encode_body(&HostEvent::GraphPublished { revision }).expect("encode graph event"),
            ))
            .expect("send graph event");
    }
    let barrier_page = create_telemetry_page(64, 2, 2).expect("barrier telemetry page");
    let barrier_writer = TelemetryWriter::map(barrier_page).expect("barrier telemetry writer");
    ...
    let first =
        decode_body::<HostEvent>(queue.front().expect("first event")).expect("decode first event");
    let last =
...
fn telemetry_page_activates_only_after_matching_active_event() {
    ...
    sender
        .send(packet(
            encode_body(&HostEvent::GraphPublished { revision: 1 }).expect("encode barrier event"),
        ))
...
fn expire_pending_and_reject_all_are_noops_for_empty_maps() {
    ...
    reject_all(&pending, failure("audio-host request", "client closed"));
    assert!(pending.lock().expect("pending lock").is_empty());
}
