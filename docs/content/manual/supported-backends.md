...
description: Check which audio backends and plug-in formats YADAW supports on each operating system.
vstTrademark: true
asioTrademark: true
---
...
YADAW uses the word **backend** for the connection between its audio engine and
...
in YADAW but still appear unavailable when its driver or device cannot be
opened.
...
ASIO presents one driver as the input and output device. YADAW therefore cannot
combine the input of one ASIO driver with the output of another. WASAPI,
CoreAudio, and ALSA can select input and output devices independently; YADAW
...
The current catalog scans only VST 3 locations and includes YADAW's built-in
Gain, Sine, and Metronome processors as VST 3 plug-ins. Installing only the
Audio Unit edition of a product will not make it appear in YADAW.

::: tip Linux editor windows
On Linux under Wayland, a plug-in's native VST 3 editor may be unavailable.
YADAW can fall back to its generic parameter editor; the native Wayland editor
