...
const observationDelayMs = Number.parseInt(process.env.YADAW_EDITOR_SMOKE_DELAY_MS ?? "250", 10)
if (!Number.isFinite(observationDelayMs) || observationDelayMs < 0 || observationDelayMs > 60_000) {
  throw new Error("YADAW_EDITOR_SMOKE_DELAY_MS must be between 0 and 60000")
}
const client = new AudioHostIpcClient(
  helperPath,
  resolve(tmpdir(), `yadaw-vst3-editor-${process.pid}.marker`),
  2,
  4,
  2
)
...
try {
  ...
  if (native.type !== "plugin-editor" || !native.open || native.active_mode !== "native") {
  ...
  if (!focused.open || focused.active_mode !== native.active_mode) {
  ...
  await new Promise((resolveWait) => setTimeout(resolveWait, observationDelayMs))
  await request({ type: "close-plugin-editor", instance_id: "editor-smoke" })
  console.log(`VST3 editor smoke passed (native=${native.active_mode}, parameter zoom=200%)`)
} finally {
  client.close()
}
