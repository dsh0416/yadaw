...
const progressLabel = computed(() => progress.value === null ? null : `${Math.round(progress.value)}%`)
...
<template>
  <section class="operation-dialog" role="dialog" aria-modal="true" :aria-labelledby="`operation-${operation.id}`">
    ...
    <div class="progress-track" :class="{ indeterminate: progress === null && operation.state === 'running' }" role="progressbar" :aria-label="statusLabel" :aria-valuenow="progress ?? undefined" aria-valuemin="0" aria-valuemax="100">
...
<style scoped>
.operation-dialog{width:min(460px,calc(100vw - 40px));padding:24px;border:1px solid var(--line-strong);border-radius:12px;color:var(--text-primary);background:#111824;box-shadow:0 28px 80px #000c}.operation-kicker{color:var(--accent);font:700 7px var(--font-utility);letter-spacing:.16em}.operation-dialog h2{margin:8px 0 6px;font:600 18px var(--font-display)}.operation-dialog p{margin:0;color:var(--text-muted);font-size:10px}.operation-status{display:flex;align-items:center;justify-content:space-between;gap:16px}.operation-status span{color:var(--text-secondary);font:700 9px var(--font-utility);font-variant-numeric:tabular-nums}.progress-track{height:7px;margin:20px 0;border-radius:999px;background:#080d15;overflow:hidden}.progress-track span{display:block;width:38%;height:100%;border-radius:inherit;background:linear-gradient(90deg,var(--accent),var(--signal-cyan));transition:width .18s ease-out}.progress-track.indeterminate span{animation:operation-progress 1.1s ease-in-out infinite}.operation-message{margin-top:12px!important}.operation-message.failed,.dropout-warning{color:#ff9dab!important}.operation-actions{display:flex;justify-content:flex-end;margin-top:20px}.operation-actions button{padding:8px 12px;border:1px solid var(--line-strong);border-radius:7px;color:var(--text-secondary);background:var(--surface-3);cursor:pointer}@keyframes operation-progress{from{transform:translateX(-110%)}to{transform:translateX(300%)}}
@media(prefers-reduced-motion:reduce){.progress-track span{transition:none}.progress-track.indeterminate span{animation:none}}
</style>
