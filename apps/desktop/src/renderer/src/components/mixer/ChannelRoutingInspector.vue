...
const sends = computed(() => (channel.value ? mixerStore.sendsFor(channel.value.id) : []))
const outputs = computed(() => (channel.value ? mixerStore.availableOutputs(channel.value.id) : []))
...
const instrument = computed(() => {
  ...
  return (
    mixerStore.graph.plugins.find(
      (plugin) => plugin.channelId === channel.value?.id && plugin.role === "instrument"
    ) ?? null
  )
})
    ...
    <template v-if="channel">
      <section class="identity-section">
        ...
        <label>
          ...
          <input
            ...
            @change="
              mixerStore.updateChannel(channel.id, { color: stringValue($event).toUpperCase() })
            "
          />
        </label>
      </section>
      ...
      <section v-if="channel.kind === 'audio'">
        <div class="section-heading">
          <span>INPUT</span><b>{{ channel.recordArmed ? "ARMED" : "SAFE" }}</b>
        </div>
        <label>
      ...
      <section v-if="channel.kind === 'output'">
        ...
        <div class="input-grid">
          <label v-for="(_, index) in channel.hardwareOutputChannels" :key="index">
            ...
            >
              <option v-for="output in inputOptions" :key="output" :value="output">
                Output {{ output }}
              </option>
            </select>
          </label>
      ...
      >
        <div class="section-heading">
          <span>SENDS</span><b>{{ sends.length }}</b>
        </div>
        <article v-for="send in sends" :key="send.id" class="send-card">
          <div class="send-card-head">
            ...
            <button aria-label="Delete send" @click="mixerStore.deleteSend(send.id)">
              <X :size="12" />
            </button>
          </div>
          <div class="send-mode">
          ...
          <label>
            <span
              >Level <b>{{ send.levelDb <= -90 ? "−∞" : `${send.levelDb.toFixed(1)} dB` }}</b></span
            >
            <input
              type="range"
              min="-90"
      ...
      <button class="clear-clips" @click="clearMeterClips">Clear meter clips</button>
    </template>
...
.routing-inspector > header span,
.routing-inspector > header strong {
  display: block;
  grid-column: 1;
}
.routing-inspector > header span {
...
.routing-inspector > header button {
  grid-column: 2;
  grid-row: 1/3;
  display: grid;
  place-items: center;
  ...
  height: 27px;
  padding: 0;
  border: 1px solid color-mix(in srgb, var(--record) 32%, var(--line-strong));
  border-radius: 4px;
  color: var(--record);
  background: color-mix(in srgb, var(--record) 10%, var(--daw-control));
  cursor: pointer;
}
.routing-inspector section {
  display: grid;
  gap: 9px;
  padding: 13px;
...
.routing-inspector label > span {
  display: flex;
  justify-content: space-between;
}
.routing-inspector label b {
  color: var(--text-secondary);
  font-weight: 500;
}
.routing-inspector input:not([type="range"]),
.routing-inspector select {
  min-width: 0;
  height: 29px;
  padding: 0 7px;
...
.routing-inspector input[type="color"] {
  width: 100%;
  padding: 3px;
}
.routing-inspector input[type="range"] {
  width: 100%;
...
.send-card-head button.enabled {
  border-color: color-mix(in srgb, var(--signal-cyan) 55%, var(--line-strong));
  color: var(--signal-cyan);
  background: color-mix(in srgb, var(--signal-cyan) 10%, var(--daw-control));
}
.send-card-head select {
  height: 25px;
}
.send-mode {
  display: grid;
...
.routing-inspector button:focus-visible,
.routing-inspector select:focus-visible,
.routing-inspector input:focus-visible {
