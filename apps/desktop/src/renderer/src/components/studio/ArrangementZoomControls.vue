...
<style scoped>
.zoom-controls,.zoom-group{display:flex;align-items:center}.zoom-controls{gap:10px}.zoom-group{gap:2px}.zoom-group>span{margin-right:3px;color:var(--text-faint);font:6px var(--font-utility);letter-spacing:.09em}.zoom-group button{display:grid;place-items:center;height:20px;min-width:20px;padding:0;border:1px solid var(--line-strong);color:var(--text-muted);background:var(--daw-control);cursor:pointer}.zoom-group button:first-of-type{border-radius:4px 0 0 4px}.zoom-group button:last-of-type{border-radius:0 4px 4px 0}.zoom-group button:hover,.zoom-group button:focus-visible{z-index:1;border-color:var(--accent-strong);color:var(--text-primary);background:var(--daw-control-hover);outline:none}.zoom-group .zoom-value{min-width:48px;padding:0 5px;color:var(--text-secondary);font:6px var(--font-utility)}
</style>
