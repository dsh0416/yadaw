...
import { Circle, LogOut, Play, Save, Settings, SkipBack, SlidersHorizontal } from "@lucide/vue"
import { TooltipArrow, TooltipContent, TooltipPortal, TooltipRoot, TooltipTrigger } from "reka-ui"
import type { NativeEngineInfo, ProjectConfiguration } from "@yadaw/contracts"

defineProps<{ nativeInfo?: NativeEngineInfo; engineRunning: boolean; project: ProjectConfiguration; recording: boolean; dirty: boolean }>()
const emit = defineEmits<{ openPreferences: []; toggleRecording: []; save: []; close: []; openProjectSettings: [] }>()
</script>
  ...
  <header class="topbar">
    ...
    <div class="transport" aria-label="Transport controls">
      <div class="transport-buttons">
        <TooltipRoot>
          <TooltipTrigger as-child><button aria-label="Go to start"><SkipBack :size="15" /></button></TooltipTrigger>
        ...
        <TooltipRoot>
          <TooltipTrigger as-child><button aria-label="Play" class="play" disabled><Play :size="15" fill="currentColor" /></button></TooltipTrigger>
          <TooltipPortal><TooltipContent class="tooltip-content" :side-offset="9">Native transport pending <span>Space</span><TooltipArrow class="tooltip-arrow" /></TooltipContent></TooltipPortal>
        </TooltipRoot>
        <TooltipRoot>
          <TooltipTrigger as-child><button aria-label="Record" :class="['record', { active: recording }]" :disabled="!engineRunning" @click="emit('toggleRecording')"><Circle :size="12" fill="currentColor" /></button></TooltipTrigger>
      ...
      <div class="time-display"><span>BAR</span><strong>001·01·000</strong></div>
