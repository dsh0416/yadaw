...
<template>
  <section class="recording-preferences">
    <div class="settings-intro">
      <span class="section-kicker">Audio <b>/</b> Recording</span>
      <h2>Recording</h2>
      <p>Capture stays in machine-local swap until a successful project archive save.</p>
    </div>
    <div class="recording-setting">
      <div>
        <h3>Swap directory</h3>
        <p>Half-finished recordings and recoverable source BWF files live here.</p>
      </div>
      <div class="path-control">
        <code>{{ settings?.swapDirectory ?? "Loading…" }}</code
        ><button :disabled="loading" @click="settingsStore.chooseSwapDirectory">Browse…</button
        ><button :disabled="loading" @click="openSwapDirectory">Open</button>
      </div>
    </div>
    <div class="recording-setting">
      <div>
        <h3>Final bit depth</h3>
        <p>Swap is always float32. Integer output is dithered once after resampling.</p>
      </div>
      <label
        >Format<select :value="settings?.recordingBitDepth" @change="setBitDepth">
    ...
    <div class="recording-setting">
      <div>
        <h3>Recovery</h3>
        <p>Files are never removed merely because the project was closed without saving.</p>
      </div>
    ...
    <p v-if="active" class="recording-note">
      A recording is active. Changes made here apply to the next recording.
    </p>
