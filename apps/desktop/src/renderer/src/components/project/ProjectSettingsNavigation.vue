...
const categories = [
  {
    id: "general",
    label: "General",
    detail: "Identity and session format",
    icon: FolderCog,
    disabled: false
  },
  { id: "timing", label: "Timing", detail: "Tempo maps and clock", icon: Clock3, disabled: true },
  {
    id: "media",
    label: "Media",
    detail: "Import and render defaults",
    icon: FileAudio,
    disabled: true
  },
  ...
  <aside class="project-settings-nav">
    ...
    <nav aria-label="Project setting categories">
      ...
      >
        ...
        <span
          ><b>{{ category.label }}</b
          ><small>{{ category.detail }}</small></span
        >
...
.project-settings-nav {
  ...
  background: linear-gradient(180deg, #111725, #0b111a);
}
.back-button {
  display: flex;
  align-items: center;
...
.nav-heading strong {
  margin-top: 7px;
  font: 560 16px var(--font-display);
}
...
.nav-item.active {
  border-color: #484771;
  color: #e0ddff;
  background: linear-gradient(90deg, #242442, #1a2030);
...
.nav-item em {
  color: #515d71;
  font: normal 6px var(--font-utility);
...
.format-rail i {
  width: 20px;
  height: 1px;
  background: linear-gradient(90deg, #5a5483, #3b7181);
}
</style>
