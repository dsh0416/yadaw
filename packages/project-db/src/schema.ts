...
export const project = pgTable("project", {
  ...
  check("project_sample_rate_check", sql`${table.sampleRate} in (44100, 48000, 88200, 96000, 176400, 192000)`),
  check("project_waveform_display_mode_check", sql`${table.waveformDisplayMode} in ('separate', 'aggregate')`)
])
...
export const assetWaveformLevels = pgTable("asset_waveform_levels", {
...
export const mixerChannels = pgTable("mixer_channels", {
  ...
  inputChannels: smallint("input_channels").array().$type<number[]>().notNull().default(sql`array[]::smallint[]`),
  hardwareOutputChannels: smallint("hardware_output_channels").array().$type<number[]>().notNull().default(sql`array[]::smallint[]`)
}, (table) => [
  foreignKey({
  ...
  uniqueIndex("mixer_master_singleton").on(table.kind).where(sql`${table.kind} = 'master'`),
  ...
  check("mixer_channels_kind_check", sql`${table.kind} in ('audio', 'instrument', 'bus', 'master', 'output')`),
...
export const timelineClips = pgTable("timeline_clips", {
  id: text("id").primaryKey(),
  assetId: text("asset_id").notNull().references(() => assets.id, { onDelete: "cascade" }),
  trackId: text("track_id").notNull().references(() => mixerChannels.id, { onDelete: "cascade" }),
  name: text("name").notNull(),
  startFrame: int8BigInt("start_frame").notNull(),
  sourceOffsetFrames: int8BigInt("source_offset_frames").notNull().default(sql`0`),
...
export const mixerSends = pgTable("mixer_sends", {
  id: text("id").primaryKey(),
  sourceChannelId: text("source_channel_id").notNull().references(() => mixerChannels.id, { onDelete: "cascade" }),
  targetChannelId: text("target_channel_id").notNull().references(() => mixerChannels.id, { onDelete: "cascade" }),
  sortOrder: integer("sort_order").notNull(),
  ...
  uniqueIndex("mixer_sends_source_target_unique").on(table.sourceChannelId, table.targetChannelId),
  ...
  check("mixer_sends_distinct_channels_check", sql`${table.sourceChannelId} <> ${table.targetChannelId}`)
])

export const pluginInstances = pgTable("plugin_instances", {
  id: text("id").primaryKey(),
  channelId: text("channel_id").notNull().references(() => mixerChannels.id, { onDelete: "cascade" }),
  ...
  componentState: bytea("component_state").notNull().default(sql`''::bytea`),
  controllerState: bytea("controller_state").notNull().default(sql`''::bytea`)
}, (table) => [
  uniqueIndex("plugin_instances_channel_role_slot_unique").on(table.channelId, table.role, table.slotOrder),
  ...
  check("plugin_instances_instrument_slot_check", sql`${table.role} <> 'instrument' or ${table.slotOrder} = 0`)
])
...
export const timeSignatureEvents = pgTable("time_signature_events", {
  ...
  check("time_signature_events_denominator_check", sql`${table.denominator} in (1, 2, 4, 8, 16, 32)`)
])
...
export const midiClips = pgTable("midi_clips", {
  id: text("id").primaryKey(),
  sourceId: text("source_id").notNull().references(() => midiSources.id, { onDelete: "restrict" }),
  trackId: text("track_id").notNull().references(() => mixerChannels.id, { onDelete: "cascade" }),
  name: text("name").notNull(),
...
export const midiNotes = pgTable("midi_notes", {
  id: text("id").primaryKey(),
  clipId: text("clip_id").notNull().references(() => midiClips.id, { onDelete: "cascade" }),
  startTick: int8Number("start_tick").notNull(),
...
export const midiEvents = pgTable("midi_events", {
  ...
  check("midi_events_channel_check", sql`${table.channel} is null or ${table.channel} between 0 and 15`),
  check(
